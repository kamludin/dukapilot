...
if 'report_data' not in st.session_state:
    st.session_state.report_data = load_data()

# Modified add_products function to save data
def add_products(new_products):
    ...
    save_data(st.session_state.report_data)  # Save to file
...
if menu == "Dashboard":
    ...
    if st.button("Clear Today's Sales", key="clear_today", type="primary"):
        if not st.session_state.products.empty:
            ...
            # Update the saved data file as well
            save_data(st.session_state.report_data)
