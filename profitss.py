...
st.set_page_config(
    page_title="Best profit tracker",
    page_icon="📊",
    layout="wide",
...
CLEARED_FLAG_FILE = "sales_cleared.flag"
...
def is_today_cleared():
    """Check if today's sales have been cleared"""
    if os.path.exists(CLEARED_FLAG_FILE):
        with open(CLEARED_FLAG_FILE, 'r') as f:
            cleared_date = f.read().strip()
        return cleared_date == date.today().strftime("%Y-%m-%d")
    return False

def set_today_cleared(cleared=True):
    """Set today's cleared status"""
    if cleared:
        with open(CLEARED_FLAG_FILE, 'w') as f:
            f.write(date.today().strftime("%Y-%m-%d"))
    elif os.path.exists(CLEARED_FLAG_FILE):
        os.remove(CLEARED_FLAG_FILE)
...
def add_products(new_products):
    ...
    # Reset cleared flag when new sales are added
    set_today_cleared(False)
...
if menu == "Dashboard":
    ...
    if not is_today_cleared() and today_profit > 0:
        st.markdown(f"""
    ...
    if not low_stock_items.empty:
        ...
        st.dataframe(
            alert_df.style.map(style_low_stock),
    ...
    if not st.session_state.products.empty and not is_today_cleared():
        ...
        st.info("No inventory data available or today's sales have been cleared.")
    ...
    <div style="background: #f8fafc; padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 1px solid #e2e8f0;">
        <p style="font-size: 1.05rem; color: #64748b; margin-bottom: 15px;">
            This action will clear today's sales from dashboard and inventory while preserving all data in reports.
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    if st.button("Clear Today's Sales", key="clear_today", type="primary"):
        set_today_cleared(True)
        st.session_state.products['Quantity Sold'] = 0  # Reset sold quantities
        save_products_data(st.session_state.products)
        st.success("Today's sales and inventory cleared from dashboard. All report data preserved.")
        st.rerun()
